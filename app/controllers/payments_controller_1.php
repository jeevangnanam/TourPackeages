...
class PaymentsController extends AppController {
	...
	function add111() {
		...
		if (!empty($this->data)) {
			...
			$user_ar = $this->Auth->user();
			$this->data['Payment']['merchent_reference'] =  substr($user_ar['User']['name'],0,3).time();

			$invoice = file_get_contents("http://localhost/seylan/pgresponse.php?amount=".$this->data['Payment']['amount']."&merchent_reference=".$this->data['Payment']['merchent_reference']);
